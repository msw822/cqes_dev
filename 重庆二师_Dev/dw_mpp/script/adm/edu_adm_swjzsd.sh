...
base=$(dirname $0)
conn=$1
sql="truncate table adm.edu_adm_swjzsd;
insert into adm.edu_adm_swjzsd(xh, sw_jzsd)
select temp2.xh,concat(concat(temp2.swsj,'-'),temp2.xwsj) as swjzsd
from (
select temp1.xh, temp1.swsj, temp1.xwsj, temp1.zswsc, row_number() over (partition by temp1.xh order by temp1.zswsc desc) as rank
from( 
