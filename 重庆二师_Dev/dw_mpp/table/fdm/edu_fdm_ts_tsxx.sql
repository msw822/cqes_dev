...
IF NOT EXISTS fdm.edu_fdm_ts_tsxx (
	...
	`TSTXM` VARCHAR (30)  COMMENT '图书条形码',
	`ZBT` VARCHAR (180)  COMMENT '正标题,填写中文或英文',
	`BLBT` VARCHAR (180) COMMENT '并列标题',
	`FBT` VARCHAR (180) COMMENT '副标题',
	`JSGJZ` VARCHAR (30) COMMENT '检索关键字',
	`SHH` VARCHAR (30) COMMENT '索书号',
	`TSCBH` VARCHAR (30)  COMMENT '图书相应的ISBN 等标准出版号',
	`DYZZ` VARCHAR (90)  COMMENT '第一作者名称',
	...
	`YZM` VARCHAR (3) COMMENT '语种码GB/T 4880.1，见[edu_hb_yz]',
	...
	`WXLX` VARCHAR(8) COMMENT '文献类型',
	`CS` INT (4) COMMENT '册数',
	`KWJS` int(4) COMMENT '可外借数',
  `YWJS` int(4) COMMENT '已外借数',
  `YYS` int(4) COMMENT '预约数',
  `TXYS` int(4) COMMENT '图象页数',
  `SYWJCS` int(4) COMMENT '上月外借册数',
  `BYWJCS` int(4) COMMENT '本月外借册数',
  `QNWJCS` int(4) COMMENT '去年外借册数',
  `JNWJCS` int(4) COMMENT '今年外借册数',
  `LJWJCS` int(4) COMMENT '累计外借册数'
