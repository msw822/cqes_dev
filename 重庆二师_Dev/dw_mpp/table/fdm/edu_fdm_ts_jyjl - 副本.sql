...
IF NOT EXISTS fdm.edu_fdm_ts_jyjl (
  ...
  `JSRQ`	VARCHAR(30) COMMENT '结束时间 YYYY-MM-DD HH:mm:ss',
  `YHRQ`	VARCHAR(30) COMMENT '归还时间 YYYY-MM-DD HH:mm:ss',
  `DZXM`	VARCHAR(40) COMMENT '读者姓名',
  `WJSJ`	VARCHAR(30) COMMENT '外借时间 YYYY-MM-DD HH:mm:ss',
  `XJCS`	tinyint(3) COMMENT '续借次数',
  `CSBM`	VARCHAR(20) COMMENT '库室编码',
  `CSMC`	VARCHAR(20) COMMENT '库室名称',
   `DT`	    VARCHAR(10) COMMENT '增量字段'
